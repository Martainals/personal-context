...
ASR 原文中的标点会在逐词对齐后按字符相似度安全贴回；相似度不足时宁可保留无标点结果，也不增删或猜测原词。最终组装在句末标点或至少 0.8 秒停顿处断段，避免把长段对话连成一整段。若 240 秒 ASR 分块出现灾难性的单字循环，Provider 只把该块切成 30 秒小块重新识别和逐块对齐；恢复结果使用独立缓存身份。任一小块仍异常时必须停止，不得发布逐字稿。
